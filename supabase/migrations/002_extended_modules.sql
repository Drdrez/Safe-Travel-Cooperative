-- ============================================================================
-- Safe Travel Cooperative — Extended modules (HR, Payroll, Members, Loans,
--                                              Maintenance, Driver perf)
-- ============================================================================
-- Run AFTER 001_production_schema.sql. Idempotent: every object uses IF NOT
-- EXISTS / CREATE OR REPLACE / guarded DO blocks so re-running is safe.
--
-- Covers the SIA-1 subsystems that 001 did not: §3.1 HR, §3.2 Payroll, the
-- cooperative-member + loan view of §2.1, vehicle maintenance from §3.3, and
-- the driver-performance slice of §2.1.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- A. Role vocabulary: add 'member' and 'driver' to the role options.
-- Employees continue to use 'admin' or 'dispatcher'. Customers stay 'customer'.
-- A single profile may be both customer AND cooperative member — that is
-- modelled via the cooperative_members table below, not via role.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- B. HR: extend profiles with employment fields for admin/dispatcher/driver.
-- These are null for customer rows and filled in by the HR screen.
-- ---------------------------------------------------------------------------
alter table public.profiles
...
-- ---------------------------------------------------------------------------
-- C. Payroll: periods + line items.
-- A period is declared by an admin ("Sep 1-15 2026"). The admin then either
-- auto-generates items (one per active employee, populated from their rate)
-- or adds them manually. Once status='Posted' the employee can view the
-- payslip and the totals are locked.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- D. Cooperative membership + contributions + loans.
--
-- Per SIA-1, a member is a coop member (shareholder / contributor), not the
-- same thing as a paying customer. A customer may or may not be a member; we
-- keep the relationship via profile_id so an existing user can "become" one.
-- ---------------------------------------------------------------------------
...
-- Auto-generate a member_number like 'MBR-2026-00042' on insert when blank.
...
-- Contributions: monthly dues, share capital top-ups, etc.
...
-- Loan requests: member asks for a loan, admin approves/rejects and later
-- marks disbursed. Amortization schedule is computed at disbursement.
...
-- When a loan transitions to 'Disbursed', seed balance_cents from the amount.
...
-- Loan payments against an active loan.
...
-- After each payment, reduce the loan balance and auto-close on full payoff.
...
-- ---------------------------------------------------------------------------
-- E. Vehicle maintenance records.
-- Admin logs or schedules service. While an 'Open' maintenance record exists,
-- the related vehicle is forced to status='Maintenance' so it can't be rented.
-- ---------------------------------------------------------------------------
...
-- Flip vehicle status automatically when a maintenance record opens/closes.
...
begin
  if tg_op = 'INSERT' and new.status in ('Scheduled', 'In Progress') then
    ...
    if new.status in ('Scheduled', 'In Progress') and old.status not in ('Scheduled', 'In Progress') then
      ...
      -- Only auto-flip back if there are no other open records for the vehicle.
      if not exists (
...
-- ---------------------------------------------------------------------------
-- F. Driver performance view.
-- Small, read-only, indexed roll-up used by the Driver profile page.
-- ---------------------------------------------------------------------------
...
-- Any employee with a posted item can see the period header for that item.
...
-- Cooperative members / contributions / loans ------------------------------
...
-- A member may submit their own loan request; admin controls approval.
...
-- Loan payments (member can view their own; only admin records them)
...
create policy "maint admin all"
  ...
  using (public.is_admin()) with check (public.is_admin());
-- Every authenticated user can see maintenance for a vehicle (used by the
-- customer app to show "Vehicle unavailable until <date>" tooltips).
create policy "maint auth read"
  on public.maintenance_records for select
  using (auth.role() = 'authenticated');

-- ============================================================================
-- Notification triggers for the new modules
-- ============================================================================

-- Loan decision (approved / rejected) -> notify the member.
...
-- New loan request -> notify all admins.
...
-- Payroll posted -> notify each employee whose item is in the period.
...
-- Maintenance scheduled/completed -> admin log only (no customer fan-out
-- needed; customers see it implicitly via vehicle availability).
...
-- Audit loans (created, status changed, disbursed)
...
-- ============================================================================
-- Schema reload so PostgREST sees the new tables/relationships immediately.
-- ============================================================================
