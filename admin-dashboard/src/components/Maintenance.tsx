...
export function Maintenance() {
  ...
  useRealtimeRefresh('maintenance_records', () => { void fetchRecords(); });
  ...
  const dueSoon = useMemo(() => records.filter(r => {
    if (!r.scheduled_for || r.status === 'Completed' || r.status === 'Cancelled') return false;
    const ms = new Date(r.scheduled_for).getTime() - nowMs;
    return ms < 1000 * 60 * 60 * 24 * 7 && ms > 0;
  }).length, [records, nowMs]);
